const CACHE='fieldverify-pro-v111-full-ncr-window';
const REQUIRED_BUILD='10.11';
const CORE=[
  './','./index.html','./ncr-ui-patch.js','./ncr-import-fix-v108.js','./ncr-engineer-fix-v109.js','./ncr-full-window-v1011.js','./caisson-plan.png','./caisson-data.js',
...
async function patchHtml(response){
  ...
  patched=patched.replace(/v(?:7\.3|7\.5|7\.6|10\.4|10\.5|10\.6|10\.7|10\.8|10\.9|10\.10)\s+stable/gi,'v10.11 stable');
  const tags=[
    '<script src="./ncr-ui-patch.js?v=10.11"></script>',
    '<script src="./ncr-import-fix-v108.js?v=10.11"></script>',
    '<script src="./ncr-engineer-fix-v109.js?v=10.11"></script>',
    '<script src="./ncr-full-window-v1011.js?v=10.11"></script>'
  ];
  for(const tag of tags){
    const file=tag.match(/src="\.\/(.*?)\?/)[1];
    const re=new RegExp(file.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(?:\\?v=[^"\\\'<> ]+)?','g');
    if(patched.includes(file)) patched=patched.replace(re,file+'?v=10.11');
    else patched=injectBeforeRealBodyClose(patched,tag);
  }
  patched=patched.replace(/ncr-engineer-fix-v108\.js(?:\?v=[^"\'<> ]+)?/g,'ncr-engineer-fix-v109.js?v=10.11');
...
self.addEventListener('fetch',event=>{
  ...
  if(/\/(ncr-ui-patch|ncr-import-fix-v108|ncr-engineer-fix-v109|ncr-full-window-v1011)\.js$/.test(url.pathname)){
