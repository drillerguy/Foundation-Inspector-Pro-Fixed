(()=>{
'use strict';
const VERSION='10.25.30-ers-clean-page-numbers';
...
#fvErsCropOverlay .fv-page-virtual{position:absolute;transform:translate(-50%,-50%);min-width:78px;height:48px;padding:0 8px;border:3px solid #c32727;border-radius:10px;background:#fff;color:#8e1717;font-size:20px;font-weight:900;line-height:40px;text-align:center;box-shadow:0 2px 8px #0007;pointer-events:auto;touch-action:manipulation}
...
function drawingLabel(){const f=document.getElementById('drawingFilter');const opt=f&&f.options&&f.selectedIndex>=0?f.options[f.selectedIndex]:null;return String((opt&&opt.textContent)||'').trim()}
function sheetNumber(){const s=drawingLabel().toUpperCase();const m=s.match(/ERS\s*[-_ ]?\s*(\d+)/);return m?Number(m[1]):null}
...
function drawCrop(clientX,clientY){
 ...
 if(!nums.length)say('This ERS sheet number could not be identified yet')
}
