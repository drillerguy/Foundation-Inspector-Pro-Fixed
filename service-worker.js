const CACHE='fieldverify-pro-v108-engineer-fix';
const REQUIRED_BUILD='10.8';
const CORE=[
  './','./index.html','./ncr-ui-patch.js','./ncr-import-fix-v108.js','./ncr-engineer-fix-v108.js','./caisson-plan.png','./caisson-data.js',
...
async function patchHtml(response){
  ...
  patched=patched.replace(/v(?:7\.3|7\.5|7\.6|10\.4|10\.5|10\.6|10\.7)\s+stable/gi,'v10.8 stable');
  const tags=[
    '<script src="./ncr-ui-patch.js?v=10.8"></script>',
    '<script src="./ncr-import-fix-v108.js?v=10.8"></script>',
    '<script src="./ncr-engineer-fix-v108.js?v=10.8"></script>'
  ];
  for(const tag of tags){
    const file=tag.match(/src="\.\/(.*?)\?/)[1];
    const re=new RegExp(file.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(?:\\?v=[^"\\\'<> ]+)?','g');
    if(patched.includes(file)) patched=patched.replace(re,file+'?v=10.8');
    else patched=patched.replace(/<\/body>/i,tag+'</body>');
  }
  patched=patched.replace(/ncr-import-fix-v107\.js(?:\?v=[^"\'<> ]+)?/g,'ncr-import-fix-v108.js?v=10.8');
...
self.addEventListener('fetch',event=>{
  ...
  if(/\/(ncr-ui-patch|ncr-import-fix-v108|ncr-engineer-fix-v108)\.js$/.test(url.pathname)){
