(() => {
  ...
  async function createReportPdf(scope = 'today') {
    if (reportBusy) return;
    reportBusy = true;

    const isFullProject = scope === 'all';
    const button = isFullProject
      ? $id('fullProjectPdfBtnV10')
      : ($id('todayFullPdfBtnV10') || $id('dailyReportBtn') || $id('shareLogBtn'));

    const oldText = button?.textContent ||
      (isFullProject ? 'MAKE FULL PROJECT PDF' : 'MAKE TODAY’S FULL PDF');
    ...
    try {
      ...
      const items = Object.entries(allRecords)
        .filter(([, record]) =>
          isFullProject ? true : recordHasTodayActivity(record || {}, photosById)
        )
        .sort((a, b) => Number(a[0]) - Number(b[0]));

      if (!items.length) {
        say(isFullProject ? 'No project records were found.' : 'No work was recorded today.');
        return;
      }
      ...
      const photoCount = items.reduce(
        (sum, [, record]) => sum + (
          isFullProject
            ? (record.photos || []).map(id => photosById.get(String(id))).filter(Boolean).length
            : todayPhotos(record, photosById).length
        ),
        0
      );
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(25);
      pdf.text(isFullProject ? 'Full Project' : 'Daily Foundation', 108, 72, { align: 'center' });
      ...
      pdf.setFontSize(11);
      pdf.text(
        isFullProject ? `Generated: ${new Date().toLocaleString()}` : `Report date: ${dateText}`,
        108, 124, { align: 'center' }
      );
      ...
      pdf.text(isFullProject ? 'Saved project photos' : 'Photos taken today', 146, 183, { align: 'center' });

      for (const [number, record] of items) {
        const photos = isFullProject
          ? (record.photos || []).map(id => photosById.get(String(id))).filter(Boolean)
          : todayPhotos(record, photosById);
        const lat =
        ...
        pdf.text(`${isFullProject ? 'Saved photos' : 'Photos today'}: ${photos.length}`, 10, 93);
      ...
      const name = isFullProject
        ? `${safeFileName(title)}-Full-Project-Report-${todayKeyV10()}.pdf`
        : `${safeFileName(title)}-Daily-Report-${todayKeyV10()}.pdf`;
      const file = new File([blob], name, { type: 'application/pdf' });

      await shareFile(
        file,
        isFullProject ? 'Full Foundation Inspection Project Report' : 'Today’s Foundation Inspection Report',
        isFullProject
          ? 'Complete project report with every saved caisson, picture, note, timestamp, GPS coordinate, and inspection record.'
          : 'Daily report with caisson numbers, photos, notes, timestamps, GPS coordinates, and inspection information.'
      );

      say(`${isFullProject ? 'Full project' : 'Daily'} PDF ready — ${items.length} caissons and ${photoCount} photos`);
    } catch (error) {
  ...
  function createTodayPdf() {
    return createReportPdf('today');
  }

  function createFullProjectPdf() {
    return createReportPdf('all');
  }
  ...
  function addTodayButton() {
    ...
    if (!$id('fullProjectPdfBtnV10')) {
      const fullButton = document.createElement('button');
      fullButton.id = 'fullProjectPdfBtnV10';
      fullButton.textContent = 'MAKE FULL PROJECT PDF';
      fullButton.className = 'share-log';
      fullButton.style.background = '#16803d';
      fullButton.onclick = createFullProjectPdf;
      tools.prepend(fullButton);
    }
  }
  ...
  document.addEventListener('click', event => {
    ...
    const fullProject = event.target.closest('#fullProjectPdfBtnV10');
    if (fullProject) {
      event.preventDefault();
      event.stopImmediatePropagation();
      createFullProjectPdf();
      return;
    }
