(()=>{
'use strict';
const VERSION='10.25.50-drawing-switch-queue-source-compat';
...
function allMeta(){try{const x=JSON.parse(localStorage.getItem(META_KEY)||'[]');return Array.isArray(x)?x:[]}catch{return[]}}
function metaById(id){return allMeta().find(x=>String(x.id)===String(id))||null}
...
async function resolveDrawing(id){
  const meta=metaById(id);
  let item=await settingById(id);
  let sourceId='';
  if(!item?.blob&&meta?.sourceId){
    sourceId=String(meta.sourceId);
    item=await settingById(sourceId);
  }
  if(!item?.blob)return null;
  const pageNumber=Math.max(1,Number(meta?.pageNumber||item.pageNumber||1)||1);
  const pageCount=Math.max(1,Number(meta?.pageCount||item.pageCount||1)||1);
  const name=meta?.name||meta?.baseDescription||meta?.description||item.name||'Drawing';
  return {item:{...item,name,pageNumber,pageCount},meta,sourceId};
}
...
async function loadStoredDrawingId(id,seq){
  ...
  const resolved=await resolveDrawing(id);
  if(seq!==requestSeq)return;
  if(img._customUrl){try{URL.revokeObjectURL(img._customUrl)}catch{};img._customUrl=null}
  if(!resolved?.item?.blob){
    img.src='caisson-plan.png';img.alt='Foundation caisson plan';img.dataset.fvDrawingId='';img.dataset.fvDrawingSourceId='';return;
  }
  ...
  img.dataset.fvDrawingSourceId=resolved.sourceId||'';
...
try{
  if(typeof applyStoredDrawing==='function'&&!applyStoredDrawing._fvQueued102550){
    ...
    applyStoredDrawing._fvQueued102550=true;
  }
