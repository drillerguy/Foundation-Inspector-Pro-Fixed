(()=>{
'use strict';
const VERSION='10.25.19-photo-upload-fix-2';
...
async function objectExists(c,path){try{const dl=await c.storage.from('fieldverify').download(path);return !!(dl&&!dl.error&&dl.data&&dl.data.size>0)}catch{return false}}
async function uploadOne(c,id,p,index,total){
 ...
 say(`Cloud: uploading photo ${index}/${total}`);
 let stored=false,lastErr=null;
 for(let attempt=1;attempt<=3&&!stored;attempt++){
  try{
   const up=await c.storage.from('fieldverify').upload(path,body,{upsert:true,contentType:t,cacheControl:'3600'});
   if(up.error)throw up.error;
   stored=true;
  }catch(e){
   lastErr=e;
   if(await objectExists(c,path)){stored=true;break}
   if(attempt<3)await sleep(450*attempt);
  }
 }
 if(!stored)throw lastErr||Error('Photo could not be stored in cloud');
 const meta=await c.from('fieldverify_photos').upsert({id:String(p.id),project_id:id,item_key:item,storage_path:path,file_name:p.name||'',mime_type:t,captured_at:p.date||new Date().toISOString(),lat:p.lat??null,lon:p.lon??null,metadata:{materializedUpload:true,source:'ios-safe-upload',verifiedStored:true}},{onConflict:'id'});
...
async function uploadMissing(show=true){
 ...
 try{
  ...
  for(let i=0;i<pending.length;i++){
   try{await uploadOne(c,id,pending[i],i+1,pending.length);uploaded++}
   catch(e){failed++;errors.push(`${pending[i]?.id||i+1}: ${e?.message||e}`);console.warn('FieldVerify photo skipped after retries',pending[i]?.id,e)}
  }
  if(failed){
   say(`Cloud: ${uploaded} uploaded · ${failed} need retry`);
   console.warn('FieldVerify photo upload failures',errors);
  }else if(show)say(`Cloud: ${uploaded} photo${uploaded===1?'':'s'} uploaded`);
  return uploaded;
 }catch(e){console.error('FieldVerify robust photo upload',e);if(show)say(`Cloud photo upload failed: ${e.message||e}`);return 0}finally{busy=false}
}
async function prepareThen(original,ctx,args){await uploadMissing(true);return original?.apply(ctx,args||[])}
...
function hookSyncButton(e){const b=e.target?.closest?.('#fvSyncNow');if(!b||b.dataset.fvPhotoSafeBusy==='1')return;const old=b.onclick;if(typeof old!=='function')return;e.preventDefault();e.stopImmediatePropagation();b.dataset.fvPhotoSafeBusy='1';const txt=b.textContent;b.disabled=true;b.textContent='Uploading photos…';(async()=>{try{await uploadMissing(true);b.disabled=false;b.textContent=txt;await old.call(b,e)}finally{b.disabled=false;b.textContent=txt;delete b.dataset.fvPhotoSafeBusy}})()}
function wrapSavePhotos(){try{if(window.__fvPhotoUploadSaveWrapped||typeof savePhotos!=='function')return;window.__fvPhotoUploadSaveWrapped=true;const old=savePhotos;savePhotos=async function(...a){const r=await old.apply(this,a);setTimeout(()=>uploadMissing(false),150);return r}}catch{}}
document.addEventListener('click',hookSyncButton,true);
addEventListener('online',()=>uploadMissing(false));
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&Date.now()-lastRun>5000)uploadMissing(false)});
setInterval(()=>{wrapHosted();wrapSavePhotos()},1000);
setTimeout(()=>{wrapHosted();wrapSavePhotos();uploadMissing(false)},1200);
