...
(()=>{
'use strict';
const VERSION='10.24-cloud-photo-accelerator-3';
...
async function cleanInvalidLocalPhotos(){
 if(typeof openDB!=='function')return 0;
 const db=await openDB(),readTx=db.transaction('photos','readonly'),store=readTx.objectStore('photos'),rows=await req(store.getAll());await done(readTx);
 const bad=(rows||[]).filter(p=>p?.id&&p.blob&&typeof p.blob.size==='number'&&p.blob.size<=0);
 if(!bad.length)return 0;
 const tx=db.transaction('photos','readwrite'),out=tx.objectStore('photos');for(const p of bad)out.delete(String(p.id));await done(tx);
 console.warn(`FieldVerify removed ${bad.length} zero-byte local photo entr${bad.length===1?'y':'ies'} before sync`);
 try{toast(`${bad.length} empty photo entr${bad.length===1?'y':'ies'} repaired before cloud sync`)}catch{}
 return bad.length;
}
...
async function guardManualSync(e){
 const b=e.target?.closest?.('#fvSyncNow');if(!b||b.dataset.fvSyncGuardBypass==='1'||syncGuardBusy)return;
 const original=b.onclick;if(typeof original!=='function')return;
 e.preventDefault();e.stopImmediatePropagation();syncGuardBusy=true;b.disabled=true;const oldText=b.textContent;b.textContent='Checking photos…';
 try{await cleanInvalidLocalPhotos();b.dataset.fvSyncGuardBypass='1';b.disabled=false;b.textContent=oldText;try{await original.call(b,e)}finally{delete b.dataset.fvSyncGuardBypass}}
 catch(err){console.error('Cloud sync guard',err);try{toast(`Cloud sync preparation failed: ${err.message||err}`)}catch{}}
 finally{syncGuardBusy=false;b.disabled=false;if(b.textContent==='Checking photos…')b.textContent=oldText}
}
