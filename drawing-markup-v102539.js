(()=>{
'use strict';
const VERSION='10.25.52-drawing-markup-eraser';
...
function save(silent=false){const all=readAll();all[drawingKey()]=strokes;try{localStorage.setItem(STORE,JSON.stringify(all));if(!silent)say('Drawing markup saved')}catch{if(!silent)say('Could not save drawing markup')}}
...
#fvMarkupTools{position:absolute;right:78px;top:10px;left:auto;z-index:35;display:none;gap:6px;align-items:center;flex-wrap:wrap;padding:7px;background:#fffffff5;border-radius:12px;box-shadow:0 2px 12px #0005;line-height:normal;max-width:calc(100% - 96px)}
#fvMarkupTools.show{display:flex}
#fvMarkupTools button{min-height:42px;padding:8px 12px;background:#083a73;color:#fff;border-radius:9px;font-size:13px;font-weight:900}
#fvMarkupTools button.active{background:#b42318}
#fvMarkupTools .light{background:#e7edf4;color:#16202a}
#fvMarkupTools .danger{background:#b42318;color:#fff}
@media(max-width:760px){#fvMarkupTools{right:72px;top:8px;max-width:calc(100% - 84px);gap:5px;padding:6px}#fvMarkupTools button{min-height:38px;padding:7px 9px;font-size:12px}}
...
function ensureUi(){
 ...
 let tools=document.getElementById('fvMarkupTools');if(!tools){
   tools=document.createElement('div');tools.id='fvMarkupTools';
   tools.innerHTML='<button type="button" data-tool="pen" class="active">Pen</button><button type="button" data-tool="eraser">Erase</button><button type="button" id="fvMarkupUndo" class="light">Undo Last</button><button type="button" id="fvMarkupClear" class="danger">Clear Page</button><button type="button" id="fvMarkupSave">Save</button><button type="button" id="fvMarkupDone">Done</button>';
   (map.closest('.map-wrap')||map.parentElement)?.appendChild(tools)
 }
 toggle.onclick=()=>setEnabled(!enabled);
 tools.querySelectorAll('[data-tool]').forEach(b=>b.onclick=()=>{tool=b.dataset.tool;tools.querySelectorAll('[data-tool]').forEach(x=>x.classList.toggle('active',x===b));say(tool==='eraser'?'Eraser on — drag over a mark to remove it':'Pen on')});
 tools.querySelector('#fvMarkupUndo').onclick=()=>{if(strokes.length){strokes.pop();redraw();save(true);say('Last mark removed')}};
 tools.querySelector('#fvMarkupClear').onclick=()=>{if(!strokes.length)return;if(confirm('Clear all markup from this drawing page?')){strokes=[];redraw();save(true);say('All markup cleared from this page')}};
 tools.querySelector('#fvMarkupSave').onclick=()=>save(false);
 tools.querySelector('#fvMarkupDone').onclick=()=>{save(true);setEnabled(false);say('Drawing markup saved')};
...
function pointerUp(e){if(!drawing)return;e.preventDefault();drawing=false;current=null;save(true)}
...
function strokeHit(s,p,r){const pts=s.points||[];if(!pts.length)return false;if(pts.length===1)return Math.hypot(pts[0].x-p.x,pts[0].y-p.y)<=r;for(let i=1;i<pts.length;i++)if(distPointSegment(p,pts[i-1],pts[i])<=r)return true;return false}
function eraseAt(p){const c=document.getElementById('fvMarkupCanvas');if(!c)return;const radius=28/Math.max(1,Math.min(c.width,c.height));let changed=false;for(let i=strokes.length-1;i>=0;i--){if(strokeHit(strokes[i],p,radius)){strokes.splice(i,1);changed=true}}if(changed){redraw();save(true)}}
...
ensureUi();
window.FIELDVERIFY_DRAWING_MARKUP={version:VERSION,enable:()=>setEnabled(true),disable:()=>setEnabled(false),save,load,erase:()=>{tool='eraser';setEnabled(true)}};
